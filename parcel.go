...
func (s ParcelStore) SetStatus(number int, status string) error {
	...
	_, err := s.db.Exec("UPDATE parcel SET status = :new_status WHERE number = :id", sql.Named("new_status", status), sql.Named("id", number))
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if p.Status == ParcelStatusRegistered {
		_, err = s.db.Exec("UPDATE parcel SET address = :new_address WHERE number = :id", sql.Named("new_address", address), sql.Named("id", number))
	}
